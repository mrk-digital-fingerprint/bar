...
function BrickMapController(options) {
    ...
    this.setKeySSI = (newKeySSI) => {
        keySSI = newKeySSI;
        this.keySSI = newKeySSI;
        console.log('BM C: newKeySSI', newKeySSI)
        brickStorageService.setKeySSI(newKeySSI)
    }
    ...
    this.anchorChanges = (listener) => {
        console.log('this.anchorChanges', this.keySSI, keySSI)
        ...
        moveNewDiffsToPendingAnchoringState((err) => {
            ...
            strategy.compactDiffs(pendingAnchoringDiffs, (err, brickMap) => {
                ...
                this.saveBrickMap(this.keySSI, brickMap, (err, hash) => {
                    if (err) {
                        pendingAnchoringDiffs.unshift(brickMap);
                        return endAnchoring(listener, anchoringStatus.PERSIST_BRICKMAP_ERR, err);
                    }

                    const hashLink = keyssi.createHashLinkSSI(this.keySSI.getBricksDomain(), hash, this.keySSI.getVn(), this.keySSI.getHint());
                    // TODO: call strategy.signHash() and pass the signedHash
                    console.log('BM CONTROLLER this.keySSI', this.keySSI)
                    this.addVersion(this.keySSI, hashLink, lastValidHashLink, (err) => {
                        if (err) {
