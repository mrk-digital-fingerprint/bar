...
function Archive(archiveConfigurator) {
    ...
    this.setKeySSI = (newKeySSI) => {
        self.keySSI = newKeySSI
        console.log('this.setKeySSI', newKeySSI)
        archiveConfigurator.setKeySSI(newKeySSI);
        brickMapController.setKeySSI(newKeySSI)
    }
    ...
    this.writeFile = (path, data, options, callback) => {
        ...
        Object.assign(defaultOpts, options);
        options = defaultOpts;
        console.log('----------------', path, data, self.keySSI.getIdentifier(true))
        ...
        if (options.ignoreMounts === true) {
            ...
            this.getArchiveForPath(path, (err, dossierContext) => {
                console.log(4)
                dossierContext.archive.setKeySSI(self.keySSI)
                if (err) {
                    return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Failed to load DSU instance mounted at path ${path}`, err));
                }
                if (dossierContext.readonly === true) {
